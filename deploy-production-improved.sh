...
# Improved Production Deployment Script
# Handles path issues and user permissions automatically

set -e  # Exit on any error

echo "🚀 Starting improved production deployment..."
...
# Function to fix ecosystem.config.js paths
fix_ecosystem_config() {
    echo "🔧 Fixing ecosystem.config.js paths..."
    ssh $USER@$SERVER "cd $APP_DIR && sed -i 's|/var/www/feschunterwegs|$APP_DIR|g' ecosystem.config.js && echo 'Updated ecosystem.config.js with correct paths'"
}
...
# Step 4: Fix ecosystem.config.js paths (NEW!)
fix_ecosystem_config
...
echo "   ✅ Automatically fixes ecosystem.config.js paths"
echo "   ✅ Uses correct user and directory structure"
echo "   ✅ Handles zombie processes on port 5002"
